...
**多跳 cast 值得说准确，因为它太容易被赖上。** 在 A5 上 `INT32→FP16` 会展开成 `INT32→FP32→FP16`。这条链与直接转换**逐位相同**：FP16 在 65504 以上饱和，而低于它的每个整数在 FP32 里都是精确的，所以 FP32 那一跳从不舍入，只有最后一跳舍入。只有当某个中间类型无法精确表示落在目标范围内的源值时，链式转换才会引入差异。请去 [LegalizeTileCast](../../dev/passes/14-legalize_tile_cast.md) 查你这条链属于哪一类，而不是假定那一跳就是元凶。
...
src = torch_codegen(prog)          # check_shapes=True 可在过程中断言形状
```
