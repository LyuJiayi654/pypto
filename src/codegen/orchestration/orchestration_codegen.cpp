...
class OrchestrationStmtCodegen : public CodegenBase {
 public:
  ...
  void VisitStmt_(const IfStmtPtr& if_stmt) override {
    ...
    // ``ChipTensor`` has no public default ctor, so ``ChipTensor x;`` won't compile.
    // The phi declaration for a ChipTensor return_var must be initialised with a
    // valid ChipTensor that's already in scope. Try sources in order:
    ...
    for (const auto& rv : if_stmt->return_vars_) {
      ...
      if (cpp_type == "ChipTensor") {
        INTERNAL_CHECK_SPAN(!tensor_phi_init.empty(), if_stmt->span_)
            ...
            << "phi placeholder (``ChipTensor`` has a private default ctor). "
 ...
 private:
  ...
  std::string GetCppType(const TypePtr& type) {
    ...
    // TensorType: use the concrete runtime type because C++ rejects an
    // uninitialized ``auto x;`` declaration. Yield/Assign downstream will
    // initialize or rebind it.
  ...
  /// decl one level out is ordering-inert; ``ChipTensor`` has no public default ctor,
